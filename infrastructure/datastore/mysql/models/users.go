...
type User struct {
	...
	OrganziationID int    `boil:"organziation_id" json:"organziation_id" toml:"organziation_id" yaml:"organziation_id"`
...
var UserColumns = struct {
	...
	ID:             "id",
	OrganziationID: "organziation_id",
...
var UserWhere = struct {
	ID             whereHelperint
	OrganziationID whereHelperint
	DisplayName    whereHelperstring
}{
	ID:             whereHelperint{field: "`users`.`id`"},
	OrganziationID: whereHelperint{field: "`users`.`organziation_id`"},
...
var (
	userAllColumns            = []string{"id", "organziation_id", "display_name"}
	userColumnsWithoutDefault = []string{"organziation_id", "display_name"}
...
// Organziation pointed to by the foreign key.
func (o *User) Organziation(mods ...qm.QueryMod) organizationQuery {
	queryMods := []qm.QueryMod{
		qm.Where("`id` = ?", o.OrganziationID),
	}
...
// LoadOrganziation allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (userL) LoadOrganziation(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	if singular {
		...
		args = append(args, object.OrganziationID)
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if a == obj.OrganziationID {
					continue Outer
				}
			}

			args = append(args, obj.OrganziationID)
	...
	if singular {
		...
		object.R.Organziation = foreign
		if foreign.R == nil {
			foreign.R = &organizationR{}
		}
		foreign.R.OrganziationUsers = append(foreign.R.OrganziationUsers, object)
		return nil
	}

	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.OrganziationID == foreign.ID {
				local.R.Organziation = foreign
				if foreign.R == nil {
					foreign.R = &organizationR{}
				}
				foreign.R.OrganziationUsers = append(foreign.R.OrganziationUsers, local)
				break
			}
		}
	}
...
// SetOrganziation of the user to the related item.
// Sets o.R.Organziation to related.
// Adds o to related.R.OrganziationUsers.
func (o *User) SetOrganziation(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Organization) error {
	var err error
	if insert {
	...
	o.OrganziationID = related.ID
	if o.R == nil {
		o.R = &userR{
			Organziation: related,
		}
	} else {
		o.R.Organziation = related
	}

	if related.R == nil {
		related.R = &organizationR{
			OrganziationUsers: UserSlice{o},
		}
	} else {
		related.R.OrganziationUsers = append(related.R.OrganziationUsers, o)
	}

...
func testUserToOneOrganizationUsingOrganziation(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.OrganziationID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.Organziation().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadOrganziation(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Organziation == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.Organziation = nil
	if err = local.L.LoadOrganziation(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Organziation == nil {
		t.Error("struct should have been eager loaded")
...
func testUserToOneSetOpOrganizationUsingOrganziation(t *testing.T) {
	var err error
	...
	for i, x := range []*Organization{&b, &c} {
		err = a.SetOrganziation(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.Organziation != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.OrganziationUsers[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.OrganziationID != x.ID {
			t.Error("foreign key was wrong value", a.OrganziationID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.OrganziationID))
		reflect.Indirect(reflect.ValueOf(&a.OrganziationID)).Set(zero)
		...
		if a.OrganziationID != x.ID {
			t.Error("foreign key was wrong value", a.OrganziationID, x.ID)
		}
	}
}
...
var (
	userDBTypes = map[string]string{`ID`: `int`, `OrganziationID`: `int`, `DisplayName`: `varchar`}
	_           = bytes.MinRead
)

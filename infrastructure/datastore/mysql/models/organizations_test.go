...
func testOrganizationToManyOrganziationUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	b.OrganziationID = a.ID
	c.OrganziationID = a.ID
	...
	check, err := a.OrganziationUsers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.OrganziationID == b.OrganziationID {
			bFound = true
		}
		if v.OrganziationID == c.OrganziationID {
			cFound = true
		}
	}
	...
	if err = a.L.LoadOrganziationUsers(ctx, tx, false, (*[]*Organization)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OrganziationUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.OrganziationUsers = nil
	if err = a.L.LoadOrganziationUsers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OrganziationUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testOrganizationToManyAddOpOrganziationUsers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddOrganziationUsers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ID != first.OrganziationID {
			t.Error("foreign key was wrong value", a.ID, first.OrganziationID)
		}
		if a.ID != second.OrganziationID {
			t.Error("foreign key was wrong value", a.ID, second.OrganziationID)
		}

		if first.R.Organziation != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.Organziation != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.OrganziationUsers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.OrganziationUsers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.OrganziationUsers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	organizationDBTypes = map[string]string{`ID`: `int`, `Name`: `varchar`}
	_                   = bytes.MinRead
)
